...
def main() -> None:
    ...
    if (run_dir / "PRETEST_FROZEN.json").exists():
        raise FileExistsError(run_dir / "PRETEST_FROZEN.json")
    forbidden = [
    ...
    for model_type, selected in deployment["selected"].items():
        ...
        target = checkpoint_dir / f"{model_type}_seed42{suffix[model_type]}"
        if target.exists():
    ...
    write_json_new(run_dir / "PRETEST_FROZEN.json", result)
